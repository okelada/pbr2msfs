# pbr2msfs

Yet another tool to generate PBR texture composites, etc. Mainly for MSFS.

It works for me, but no warranties.

# Usage:

	-	Bake your textures with your favorite tool. Probably in PNG format or another one supported by .NET.
	-	Select the containing folder.
	-	The program will try to identify each PBR texture based on the filename.
	-	The ARM, NRM, RGB+Alpha and inmverted Y normal textures will be generated and you can save them somewhere.


